...
# Overview 
This document serves as a guide to the UHSGT dashboard back-end . This guide is not intended to teach how to use R but to describe the main components, files, directories, update methods, and deployment startegies regarding the application
...
Please see the below image that details the dashboards files. The ensuing bulleted list describes the items and folders as seen in the image.     
...
*	Root Level *.R files
    + global.R 
      ...
      + Loads the required packages
        + If you are doing any maintenace on the application or redeploying the application you will need to make sure all of these libraries are installed on you machine and loaded for your session
      + Loads custom functions needed for the application
      + It calls two files at the end of the script after intialising the environment – data_mapper.R and data_scripts.R 
...
*	Root Level Excel Spreadsheets
    ...
    + data_source_list.xlsx
      ...
      + Augementing this spreadsheet will create data new or edit exsiting data layers 
...
*	Root Level Maintenance-Manual.* and Operation_Manual.* Files 
    + The .Rmd files are the editable manuals that create word documents when knited
...
*	handlers folder 
    ...
    + data_census_utility.R
      ...
      + The US data query information comes from the “tidycensus” tab 
        ...
        + You will need an API key to run or perfrom any development of your own
...
*	shape-files_raw_cache folder
    ...
    + This folder is specific to the data processing process executed by the developer (mike.gaunt.404@gmail.com)
      + You will likely never have to perfrom the data processing process that queries data from this folder
*	www folder 
...
# Application Build Processe
Please see the below image that details the dashboards build process. This image provides a high-level overview of the overall process flow used to build the application, the sub-processes, and the connections between scriptes and data folders. 
![](./docs/application_build_process.PNG)    

## data_source_list.xlsx Descritpion
*	General Notes:  
    ...
    + An additional process has been included in this image to repersent potential future data layer addtions to the map
    ...
    + data_source_list.xlsx is the major driver for many of the data processing processes and is queied by many of the scripts that both process the data and that build the application 
    ...
    + tidycensus tab
      ...
      + There a many columns that record different types of meta-data for each census variable (row)
        + The columns with the green accented headers are the only meta-data that is required to obtain and build the shapefiles, they are:
          + boundary: tells the API at what geograogic level to obtain the data at
          + variable: unique ID for each census variable
          + var_name: a unique name to call the variable other than it's ID
          + name: the category that the vairbale falls under 
          ...
          + notes: addtitional information that is associated with a particular layer
          ...
          + selection: contains a string of the raw column names that are written when the shape-file is first written to *application_shapefiles*
            ...
            + The string needs to contain all the column names included in tha shapefile
...
* 
    + ca_census tab
      + This spreadsheet is very similar to the tidycenus spreadsheet with some minor execpti
...
* 
    + manual tab
      ...
      + Each column is a unique attibute given a shape-file or a trigger for a process to be ran during the data processing or application building process
      + Trigger Columns:
        + to_application_shp
          + Indicates (Y) weather a shapefile should be initally processed (spatially filtered and unneeded columns removed)
          + These shape-files are taken from *shapefiles_raw_cache* to *application_shapefiles*
          + Only relevant if you are rerunning the developers processing worklfow
        + to_map_ready
          ...
          + These shape-files either will undergo manual (actually automated code written specificlaly for different shape-files) processing, be merged with other shaep-files, and/or not be processed at all
        ...
        + to_application_shp/to_map_ready combinations and what they mean
          ...
          + N/Y is a file that does not undergo inital processing but will be used in the application
        ...
        + Attribute Columns:
          ...
          + selection: contains a string of the raw column names that are written when the shape-file is first written to *application_shapefiles*
            ...
            + The string needs to contain all the column names included in tha shape-file
...
## Example: Adding Data
This is a breif overview of how data would be added to the map.
...
Assumption - the data has the attibutes "City, StationLoc, Status, State"    
...
4) Input "N" in *to_application_shp* column since the data has been preocessed already 
...
7) Input "UHSR_Stations (studied)" in *processed_name* column becuase it's folder and layer names are that
8) Input "Transporation" in the *group* column since the layer is relevant to transportation
...
11) Input "plain" in the *zcol* column if you do not want to display distinct colors for the shape-files
    + Alternatively, the features could be colored by "status" indicating if have just been studied or if they are apart of exisiting infrastrucure
12) Input notes that provide ample infomation to the user regarding this layer
13) Input "NA" for *src_url* column since the data was made manually and does not have a direct source link
14) Run the *data_mapper.R* to check weather the map is included in the applicaiton objects
